...
def parse_condition_aux(alist, negated):
    ...
    tag = alist[0]
    if tag in ("and", "or", "not", "imply","oneof"):
        args = alist[1:]
        if tag == "imply":
            assert len(args) == 2
        if tag == "not":
    ...
    if tag == "and" and not negated or tag == "or" and negated or tag =="oneof":
...
class Condition(object):
    ...
    def print_atom(self, myindex):
	parts = self.parts
	if myindex == -1:
		s = " %s" % self._dump()
		return s
	if len(parts) <= myindex:
        	if parts[0].__class__.__name__ == "Atom":
			s = " %s" % parts[0],
			return s
		elif parts[0].__class__.__name__ == "Conjunction":
			s = ""
			for newpart in parts[0].parts:
				s += " %s" %newpart
			return s
		elif parts[0].__class__.__name__ == "NegatedAtom":
			return ""							
	else:
        	if parts[myindex].__class__.__name__ == "Atom":
			s = " %s" % parts[myindex],
			return s
		elif parts[myindex].__class__.__name__ == "Conjunction":
			s = ""
			for newpart in parts[myindex].parts:
				s += " %s" %newpart
			return s							
		elif parts[myindex].__class__.__name__ == "NegatedAtom":
			return ""							
    def get_atom(self):
	return self.parts[0]
    def get_conjunction_atoms(self):
	atoms = set()
	parts = self.parts
	for part in self.parts:
		atoms.add(part),
	...
	parts = self.parts
		...
		atoms = parts[0].get_conjunction_atoms()
	return atoms
    '''def first_atoms(self):
	atoms = []
	if self.parts[0].__class__.__name__ == "Atom":
		atoms.append(parts[0])
	else:
		2ndparts = self.parts[0].parts
		for part in 2ndparts:
			atoms.append(part)
	return atoms'''
    def _dump(self):

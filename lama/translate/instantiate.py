...
def get_fluent_facts(task, model):
  ...
  for oneof in task.oneofs:
	for part in oneof.parts:
	    if isinstance(part, pddl.Atom):
		fluent_predicates.add(part)
	    else:
		for deepfact in part.parts:
	    		if isinstance(deepfact, pddl.Atom):
				fluent_predicates.add(deepfact)

  for axiom in task.axioms:
    fluent_predicates.add(axiom.name)

  return set([fact for fact in model
              if fact.predicate in fluent_predicates or fact in fluent_predicates])

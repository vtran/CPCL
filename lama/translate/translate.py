...
def translate_strips_operator(operator, dictionary, ranges, mutex_dict, mutex_ranges, implied_facts):
    ...
    for conditions, fact in operator.del_effects:
		...
		# Look for matching add effect; ignore this del effect if found.
                # if eff_condition in eff_conditions or [] in eff_conditions:
		if ranges[var] == 2:
			my_list.append((var,eff_condition))
		#continue
    ...
    for var, eff_condition in my_list:
        none_of_those = ranges[var] - 1
	pre_post.append((var, -1, none_of_those, eff_condition))
...
def pddl_to_sas(task):
	...
	write_oneof_file(translation_key,task)
...
def write_oneof_file(translation_key, task):
    ...
    if ors != []:
	 for state in belief_state:
	    ...
	    originalgoal = task.originalgoal
 	    if isinstance(originalgoal, pddl.Disjunction):
		oneof_combinable = False

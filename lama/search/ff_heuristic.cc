...
void FFHeuristic::dump(UnaryOperator *op) const {
    cout << op->op->get_name() << ":";
    cout << "[[";
...
void FFHeuristic::dump(Proposition *prop) const {
    cout << " " << g_variable_name[prop->var] << ":" << prop->val;
}
...
void FFHeuristic::build_unary_operators(const Operator &op) {
        ...
        for(int j = 0; j < precondition_var_vals2.size(); j++)
	{
		...
		if((precondition_var_vals2[j].first == precondition_var_vals2[k].first)&&(precondition_var_vals2[j].second==precondition_var_vals2[k].second))
		{
			added = true;
		}
	   if (added == false)

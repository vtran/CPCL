...
*/
//addin
void BestFirstSearchEngine::add_heuristic(bool use_estimates,
					  bool use_preferred_operators) {
    assert(use_estimates || use_preferred_operators);
    heuristics.push_back(l_ff_heur);
    best_heuristic_values.push_back(-1);
    if(use_estimates) {
	//addin: the first thread update open_lists
        {
	open_lists.push_back(OpenListInfo(l_ff_heur, false));
	open_lists.push_back(OpenListInfo(l_ff_heur, true));
  	}
    }
    if(use_preferred_operators)
	preferred_operator_heuristics.push_back(l_ff_heur);
}
...
int BestFirstSearchEngine::step() {
	...
	if(l_ff_heur != NULL)
	    l_ff_heur->set_recompute_heuristic(); 

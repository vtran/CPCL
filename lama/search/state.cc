...
State::State(const State &Copy_State)
{
    for(int i = 0;i < g_variable_domain.size(); i++) {
        vars.push_back(Copy_State.vars[i]);
    }
//    g_default_axiom_values = vars;
//    g_value = Copy_State.g_value;
}
*/
...
bool State::satisfy_subgoal(vector<pair <int, int> > sub_goal) const {
	for(int i=0;i<sub_goal.size();i++)
	{
		if(vars[sub_goal[i].first] != sub_goal[i].second)
			return false;
	}
	return true;
}
